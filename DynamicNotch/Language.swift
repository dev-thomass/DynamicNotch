...
private func relaunchApp() {
    ...
    do {
        // `Process.launch()` is deprecated; `run()` throws on a bad executable
        // URL or missing permissions, which we want to log rather than crash.
        try task.run()
    } catch {
        Log.app.error("relaunch failed: \(error.localizedDescription, privacy: .public)")
        // Fall through to exit anyway — the user just confirmed they want a
        // restart. Worse case they relaunch from the Dock manually.
    }
    exit(0)
}
